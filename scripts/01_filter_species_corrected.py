#!/usr/bin/env python3
"""
Corrected species confirmation filter.

Keep records where ANI Submitted species == Vibrio parahaemolyticus.
This fixes the overly strict Organism Name filter that excluded records with
strain or serotype suffixes in Organism Name.

No year filtering, geographic filtering, BioSample deduplication, genome
download, or deletion is performed.
"""
...
KEPT = PROJECT / "data/processed/01_species_confirmed_corrected.tsv"
EXCLUDED = PROJECT / "data/processed/01_species_excluded_corrected.tsv"
SUMMARY = PROJECT / "results/summary/06_species_filter_corrected_summary.txt"
...
def main() -> None:
    print(f"Input: {INPUT}")
    print(f"Corrected keep rule: {ANI_SPECIES_COL} == {SPECIES}")
    ...
    with SUMMARY.open("w") as handle:
        handle.write("Corrected species confirmation filter summary\n")
